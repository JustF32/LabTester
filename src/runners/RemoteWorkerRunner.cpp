...
bool labRootHasRequiredFiles(const QString &labRoot, const QString &suiteFileName)
{
    const QDir dir(labRoot);
    const QString suitePath = QStringLiteral("tests/") + (suiteFileName.isEmpty() ? QStringLiteral("sort_test.cpp") : suiteFileName);
    return dir.exists(suitePath) && dir.exists(QStringLiteral("tests/cc.h"));
}
...
QString detectLocalLabRoot(const labtester::domain::Submission &submission)
{
    ...
    QString expectedSuiteName = QFileInfo(submission.labWork.testSuitePath).fileName();
    if (expectedSuiteName.isEmpty()) {
        expectedSuiteName = QStringLiteral("sort_test.cpp");
    }

    for (const QString &candidate : candidates) {
        const QFileInfo info(candidate);
        if (info.exists() && info.isDir() && labRootHasRequiredFiles(info.absoluteFilePath(), expectedSuiteName)) {
...
)
{
    ...
    payload->insert(QStringLiteral("testSuiteFile"), expectedSuiteName);
...
domain::TestRunResult RemoteWorkerRunner::run(const domain::Submission &submission) const
{
    try {
        ...
        QJsonObject runPayload {
            ...
            {QStringLiteral("testSuiteFile"), syncPayload.value(QStringLiteral("testSuiteFile")).toString()},
