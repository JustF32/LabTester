...
bool hasSuiteAndHeaderInDir(const QString &directoryPath, const QString &suiteFileName)
{
    const QDir dir(directoryPath);
    return dir.exists(suiteFileName)
...
UTestLayout resolveUTestLayout(const labtester::domain::Submission &submission)
{
    ...
    for (const auto &candidate : candidateRoots) {
        ...
        QString expectedSuiteName = QStringLiteral("sort_test.cpp");
        if (!submission.labWork.testSuitePath.isEmpty()) {
            expectedSuiteName = QFileInfo(submission.labWork.testSuitePath).fileName();
        }

        if (!hasSuiteAndHeaderInDir(absoluteRoot, expectedSuiteName)) {
            continue;
        }

        const QFileInfo suiteInfo(QDir(absoluteRoot).filePath(expectedSuiteName));
...
)
{
    if (layout.suitePath.isEmpty()) {
        if (errorMessage) {
            *errorMessage = QStringLiteral("Не найден файл тестов (например, cc_test.cpp).");
        }
        return QString();
    }
    const QString targetName = QFileInfo(layout.suitePath).baseName();
    ...
    cmakeScript += QStringLiteral("add_executable(%1\n").arg(targetName);
    cmakeScript += QStringLiteral("    \"%1\"\n").arg(escapedPathForCmake(suitePath));
    cmakeScript += QStringLiteral("    \"${LABTESTER_STUDENT_SOURCE}\"\n");
    cmakeScript += QStringLiteral(")\n\n");
    cmakeScript += QStringLiteral("target_include_directories(%1 PRIVATE\n").arg(targetName);
    cmakeScript += QStringLiteral("    \"%1\"\n").arg(escapedPathForCmake(suiteDir));
    cmakeScript += QStringLiteral("    \"%1\"\n").arg(escapedPathForCmake(headerDir));
    cmakeScript += QStringLiteral(")\n\n");
    cmakeScript += QStringLiteral("target_link_libraries(%1 PRIVATE GTest::gtest_main)\n\n").arg(targetName);
    cmakeScript += QStringLiteral("include(GoogleTest)\n");
    cmakeScript += QStringLiteral("gtest_discover_tests(%1)\n").arg(targetName);
...
QString resolveBuiltExecutable(const QString &buildDir, const QString &targetName)
{
    const QStringList candidates = {
        QDir(buildDir).filePath(QStringLiteral("%1.exe").arg(targetName)),
        QDir(buildDir).filePath(targetName),
        QDir(buildDir).filePath(QStringLiteral("Debug/%1.exe").arg(targetName)),
        QDir(buildDir).filePath(QStringLiteral("Release/%1.exe").arg(targetName)),
        QDir(buildDir).filePath(QStringLiteral("RelWithDebInfo/%1.exe").arg(targetName))
    };
...
CaseMetadata metadataForCase(const QString &testName)
{
    ...
    return {
        QStringLiteral("См. входные данные в исходном коде тестов"),
        QStringLiteral("См. ожидаемое поведение в исходном коде тестов")
    };
}
...
domain::TestRunResult CppGTestRunner::run(const domain::Submission &submission) const
{
    ...
    if (utestLayout.suitePath.isEmpty() || utestLayout.headerPath.isEmpty()) {
        return makeBuildErrorResult(
            ...
            QStringLiteral("Ожидались файлы тестов и cc.h для выбранной лабораторной.")
        );
    }
    ...
    const QString targetName = QFileInfo(utestLayout.suitePath).baseName();
    const QString testExecutable = resolveBuiltExecutable(buildDir, targetName);
    if (testExecutable.isEmpty()) {
        return makeBuildErrorResult(
            submission,
            QStringLiteral("Собранный исполняемый файл тестов не найден."),
            QStringLiteral("Ожидался исполняемый файл тестов (%1) в каталоге: %2").arg(targetName, buildDir)
        );
    }
