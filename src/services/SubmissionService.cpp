...
void SubmissionService::ensureCatalogLoaded()
{
    ...
    m_labWorks = m_submissionRepository.fetchLabWorks();
    // Force update the lab works with the mock data on every startup
    // to ensure test paths are correct even for existing databases
    std::vector<domain::LabWork> mockWorks = createMockLabWorks();
    m_submissionRepository.replaceLabWorks(mockWorks);
    m_labWorks = m_submissionRepository.fetchLabWorks();
...
std::vector<domain::LabWork> SubmissionService::createMockLabWorks() const
{
    ...
    auto resolveLabAssets = [&](int labId) -> LabAssetPaths {
        ...
        const QString suiteFileName = (labId == 2) ? QStringLiteral("structures_test.cpp") :
                                      (labId == 3) ? QStringLiteral("hash_test.cpp") :
                                      (labId == 4) ? QStringLiteral("strings_test.cpp") :
                                                     QStringLiteral("sort_test.cpp");

        for (const QString &candidate : candidates) {
            if (candidate.trimmed().isEmpty()) {
                continue;
            }
            const QFileInfo suiteInfo(QDir(candidate).filePath(QStringLiteral("tests/") + suiteFileName));
