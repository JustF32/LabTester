...
QString validateSyncedLabForRun(const QString &expectedLabSignature)
{
    ...
    if (!labDir.exists(QStringLiteral("tests/sort_test.cpp")) || !labDir.exists(QStringLiteral("tests/cc.h"))) {
        return QStringLiteral(
            "На воркере для тестового набора %1 лежит неполный набор файлов: нужны tests/sort_test.cpp и tests/cc.h."
        ).arg(expectedLabSignature.left(12));
    }
...
void WorkerServer::handleSyncLab(QTcpSocket *socket, const RequestData &request)
{
    ...
    if (labDir.exists()
        && labDir.exists(QStringLiteral("tests/sort_test.cpp"))
...
void WorkerServer::handleRun(QTcpSocket *socket, const RequestData &request)
{
    ...
    const QString labValidationError = validateSyncedLabForRun(input.labSignature);
...
void WorkerServer::startJob(const QString &jobId)
{
    ...
    QThreadPool::globalInstance()->start([safeThis, jobId, input]() {
        ...
        submission.labWork = domain::LabWork{
            ...
            QString(),
            QString(),
            QDir(syncedLabDir).filePath(QStringLiteral("tests/sort_test.cpp")),
